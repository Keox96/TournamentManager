...
class TeamService:
    def __init__(self, repository: AbstractTeamRepository) -> None:
    ...
    async def create_team(self, team: Team) -> Team:
        existing_name = await self.repository.get_by_name(team.name)
        if existing_name is not None:
            raise TeamNameAlreadyExistsError(details={"name": team.name})
        existing_tag = await self.repository.get_by_tag(team.tag)
    ...
    async def update_team(self, team_id: uuid.UUID, team: Team) -> Team:
        ...
        if (
            ...
            and await self.repository.get_by_name(team.name) is not None
        ):
            raise TeamNameAlreadyExistsError(details={"name": team.name})
        # Check if tag is changing and if new tag already exists
        if (
            team.tag != existing.tag
            and await self.repository.get_by_tag(team.tag) is not None
        ):
            raise TeamTagAlreadyExistsError(details={"tag": team.tag})
        # Update data
        updated_data = {
